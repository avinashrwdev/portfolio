...
export default function Header({ links }: HeaderProps){
  ...
  return(
    <header className="hidden md:flex items-center justify-center fixed z-[999] w-full mt-4">
      <motion.div className="flex p-1 rounded-none border border-white border-opacity-40 bg-white bg-opacity-80 shadow-lg shoadow-black/[0.03] backdrop:blur-[0.5rem] sm:rounded-full dark:bg-gray-950 dark:border-black/40 dark:bg-opacity-75 "
      ...
      >
        <ul className="flex flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-500">
          {
            links.map((link)=>(
              <motion.li className="flex justify-center items-center relative"
              key={link.hash}
              initial={{ y:-100, opacity:0 }}
